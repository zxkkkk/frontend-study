<template>
    <ElForm class="register-form" ref="registerRef" :model="registerParam" :rules="registerRules">
        ...
        <ElFormItem prop="email">
            <ElInput placeholder="请输入邮箱" :prefix-icon="Message" v-model="registerParam.email" size="large" />
        </ElFormItem>
        <ElFormItem>
            <ElButton type="primary" class="register-btn" size="large" @click="submit(registerRef)">注册</ElButton>
        </ElFormItem>
        <ElFormItem>
            <ElButton type="primary" class="switch-btn" size="large" @click="switchToLogin">去登录</ElButton>
        </ElFormItem>
    </ElForm>
</template>
